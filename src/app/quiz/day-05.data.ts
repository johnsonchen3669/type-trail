  ...
  questions: [
    {
      ...
      hint: '分開考慮「目前的 async function」與「整個 JavaScript 執行環境」。',
      options: [
        { id: 'A', label: 'await 會阻塞整個 JavaScript 執行環境，直到 Promise 完成' },
      ...
      correctAnswer: 'B',
      explanation:
        'await 等待 Promise 完成時，只暫停目前 async function 的後續流程，並把控制權交還執行環境。它沒有阻塞整個 JavaScript，也沒有移除 Promise。',
    },
    {
      id: 'day-05-02',
      number: 2,
      kind: 'choice',
      learningGoal: '依據同步程式、微任務與工作的順序判斷執行環境輸出',
      ...
      correctAnswer: 'C',
      explanation:
        'A、D 是目前工作中的同步輸出。這項工作結束後，事件迴圈先從微任務佇列取出 Promise 的處理函式，因此輸出 C；接著才從工作佇列取出計時器的回呼函式，輸出 B。',
    },
    {
      id: 'day-05-03',
      number: 3,
      kind: 'choice',
      learningGoal: '辨識把 HTTP 回應誤當成 API 成功的錯誤',
      prompt: '以下載入函式最大的問題是什麼？',
      hint: 'fetch 收到 404 或 500 HTTP 回應時，回傳的 Promise 不一定失敗。',
      ...
      options: [
        { id: 'A', label: 'fetch 不能放在 async function 裡' },
        {
          id: 'B',
          label:
            '程式沒有先檢查 response.ok 或 response.status，可能把 HTTP 錯誤回應當成成功資料處理',
        },
      ...
      correctAnswer: 'B',
      explanation:
        'fetch 收到 HTTP 錯誤狀態時，回傳的 Promise 通常仍會成功，所以程式應先檢查 response.ok 或狀態碼。解析 JSON 與驗證資料內容又是另一個問題。',
    },
    {
      id: 'day-05-04',
      number: 4,
      ...
      prompt:
        'Promise 尚未成功或失敗時，處於哪一個狀態？請輸入文章使用的英文狀態名稱。',
      hint: '三種狀態中，另外兩個分別表示成功與失敗；本題要找「仍在等待」的狀態。',
      acceptedAnswers: ['pending'],
      explanation: 'Promise 建立後、尚未完成時處於 pending；之後可能成功或失敗。',
    },
    {
      id: 'day-05-05',
      number: 5,
