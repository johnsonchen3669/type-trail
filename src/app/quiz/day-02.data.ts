  ...
  questions: [
    {
      ...
      options: [
        ...
        { id: 'B', label: '變數可以在 Runtime 先後指向不同型別的值' },
        { id: 'C', label: 'JavaScript 會在函式宣告時確認每個參數的型別' },
        { id: 'D', label: '動態型別表示所有運算都會自動得到符合需求的結果' },
      ],
      correctAnswer: 'B', explanation: 'JavaScript 的值有型別，但變數不會被固定在單一型別。例如先執行 let answer = 1，此時 typeof answer 是 "number"；接著執行 answer = "A"，typeof answer 就會變成 "string"。同一個變數在 Runtime 先後指向了不同型別的值。',
    },
    {
      id: 'day-02-02', number: 2, kind: 'choice', learningGoal: '判斷運算子觸發隱式型別轉換後的 Runtime 結果',
      ...
      options: [
        ...
        { id: 'D', label: '第一行與第二行都發生 Runtime error' },
      ],
      correctAnswer: 'B', explanation: '+ 在其中一邊是字串時會進行字串串接，因此得到 "51"；- 會嘗試把 "5" 轉成數字，因此得到 4。',
    },
    {
      id: 'day-02-03', number: 3, kind: 'choice', learningGoal: '辨識把 Falsy 直接當成「沒有資料」造成的需求錯誤',
      ...
      correctAnswer: 'C', explanation: '數字 0 是 Falsy。Truthy／Falsy 是 JavaScript 的轉換規則，不等於產品定義的「有答案」與「沒有答案」。',
    },
    {
      id: 'day-02-04', number: 4, kind: 'exact-text', learningGoal: '辨識無法轉成數字時的結果',
      prompt: 'Number("five") 在 JavaScript Runtime 會得到哪一個值？請輸入該值的程式碼表示法。',
      hint: 'JavaScript 有一個特殊的 number 值，用來表示這次數字轉換沒有得到有效數字。',
